...
class Prefs(context: Context) {
    ...
    private val appPrefDeviceInfo = "deviceInfoPref"
    ...
    var deviceInfoPref: DeviceSetterGetter?
        get() {
            val data = preferences.getString(appPrefDeviceInfo, null)
            if (data.isNullOrEmpty()) return null
            return gson.fromJson(data, DeviceSetterGetter::class.java)
        }
        set(value) {
            val data = gson.toJson(value).toString()
            return preferences
                .edit()
                .putString(appPrefDeviceInfo, data)
                .apply()
        }
    ...
    fun delete() {
        ...
        preferences.edit().putString(appPrefDeviceInfo, "").apply()
    }
}